...
import (
	...
	"github.com/clstb/phi/go/phi-cli/pkg/beancount"
	"github.com/clstb/phi/go/phi-cli/pkg/commands"
	models2 "github.com/clstb/phi/go/phi-cli/pkg/models"
...
func newModel(ctx *cli.Context) model {
	...
	return model{
		...
		state: state.AUTH,
		subModels: map[state.State]tea.Model{
			state.HOME:     models2.NewHome(client),
			state.AUTH:     models2.NewAuth(client),
			state.CLASSIFY: models2.NewClassify(),
			state.SYNC:     models2.NewSync(client),
		},
		client: client,
	}
}
