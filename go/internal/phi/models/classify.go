...
func (m Classify) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case beancount.Ledger:
		...
		for i, v := range msg {
			transaction, ok := v.(beancount.Transaction)
			if !ok {
				continue
			}
...
func (m Classify) View() string {
	...
	for _, prefix := range []string{
		...
		var accounts []string
		for _, account := range opens.Filter(func(open beancount.Open) bool {
	...
	Submit: key.Key{
		...
		Action: func(m tea.Model) (tea.Model, tea.Cmd) {
			...
			if len(model.matches) == 0 {
				model.ledger = append(model.ledger, beancount.Open{
					Date:    "1970-01-01",
				...
				transaction := model.ledger[unclassified.transactionIndex].(beancount.Transaction)
